						...
						<?php foreach ( $order->get_items() as $item ) : ?>
							<tr class="order_item">
								<td class="product-name">
									...
									<strong class="product-quantity">x <?php echo esc_html( $item['qty'] ); ?></strong>
								</td>
								<td class="product-total"><?php echo ent2ncr($order->get_formatted_line_subtotal( $item )); ?></td>
							</tr>
						<?php endforeach; ?>
					<?php endif; ?>
				</tbody>
				<tfoot>
					<?php if ( $totals = $order->get_order_item_totals() ) : ?>
						<?php foreach ( $totals as $total ) : ?>
							<tr>
								<th scope="row"><?php echo ent2ncr($total['label']); ?></th>
								<td ><?php echo ent2ncr($total['value']); ?></td>
							</tr>
						<?php endforeach; ?>
					<?php endif; ?>
				</tfoot>
